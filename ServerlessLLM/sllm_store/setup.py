...
class cmake_build_ext(build_ext):
    ...
    def build_extensions(self) -> None:
        ...
        self.copy_runtime_libraries()

    def copy_runtime_libraries(self) -> None:
        package_dir = os.path.abspath(
            os.path.dirname(self.get_ext_fullpath("sllm_store._checkpoint_store"))
        )
        os.makedirs(package_dir, exist_ok=True)

        search_roots = [self.build_temp, package_dir]
        for root_dir in search_roots:
            if not os.path.exists(root_dir):
                continue
            self.copy_glog_from(root_dir, package_dir)

    def copy_glog_from(self, root_dir: str, package_dir: str) -> None:
