...
def _compute_device_placement_from_map(
    ...
    if isinstance(device_map, str):
        no_split_modules = _get_no_split_modules(model, device_map)
...
def _get_no_split_modules(
    model: torch.nn.Module, device_map: str
) -> List[str]:
    get_no_split_modules = getattr(model, "_get_no_split_modules", None)
    if callable(get_no_split_modules):
        return get_no_split_modules(device_map)
